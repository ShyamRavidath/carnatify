...
for tr, te in gkf.split(X_full, y_raga, groups=groups):
    ...
    for i in te:
        ...
        top8 = clf.classes_[np.argsort(-proba)[:8]]
        pmap = dict(zip(clf.classes_, proba))
        clip_pred_top3[i] = (set(top8), clip, pmap)
...
print(f'clip({Q_SECS}s) raga top-8 accuracy (clip-trained x8): {raga_top3_hits}/{n_pred}={raga_top3_hits/n_pred:.1%}', flush=True)
...
for k, i in enumerate(eval_idx):
    top3, clip, pmap = clip_pred_top3[i]
    ...
    for j in cand:
        ...
        for rb in refs[j]:
            try:
                ...
                pass
        s[j] = best
    # blend raga probability: lower distance is better; subtract scaled proba
    ranked = sorted(s, key=lambda j: s[j] - 0.5 * pmap.get(ragas_f[j], 0.0))
...
print(f'E2E {Q_SECS}s clip, clip-trained top-8 gate + proba blend: top1={t1}/{n_eval}={t1/n_eval:.1%} top5={t5}/{n_eval}={t5/n_eval:.1%}', flush=True)
