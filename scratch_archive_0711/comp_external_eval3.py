...
# slim catalog: keep every multi-rendition track + 500 sampled distractors
multi_i = [i for i in range(N) if kc[keys[i]] >= 2]
single_i = [i for i in range(N) if kc[keys[i]] == 1]
keep_i = sorted(set(multi_i) | set(int(x) for x in rng.choice(single_i, min(500, len(single_i)), replace=False)))
paths = [paths[i] for i in keep_i]; keys = [keys[i] for i in keep_i]; chromas = [chromas[i] for i in keep_i]
N = len(paths)
kc = Counter(keys)
