...
for tr, te in gkf.split(X_full, y_raga, groups=groups):
    ...
    for i in te:
        ...
        top3 = clf.classes_[np.argsort(-proba)[:3]]
...
print(f'clip({Q_SECS}s) raga top-3 accuracy: {raga_top3_hits}/{n_pred}={raga_top3_hits/n_pred:.1%}', flush=True)
...
print(f'E2E {Q_SECS}s clip, predicted top-3 raga gate: top1={t1}/{n_eval}={t1/n_eval:.1%} top5={t5}/{n_eval}={t5/n_eval:.1%}', flush=True)
