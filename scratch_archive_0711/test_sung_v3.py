...
for path in sorted(folder.glob('*.m4a')):
    ...
    for wi in range(len(win_track)):
        ...
        if d < best_per_track.get(ti, np.inf):
            best_per_track[ti] = d
    scored = sorted(best_per_track.items(), key=lambda kv: kv[1])
    seen, results = set(), []
    for ti, d in scored:
    ...
    if '__' in path.stem:
        gtf = fold(path.stem.split('__', 1)[0])
        rank = next((k+1 for k, (t, _, _) in enumerate(results) if fold(t) == gtf), -1)
        print(f'  TRUE-WORK PURE-QMAX RANK: {rank} of {len(results)}')
    results = results[:5]
